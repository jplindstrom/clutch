...
=M-.= through the buffer-local xref backend for SQL alias navigation, so =M-.=
on =u= or =u.name= jumps to the alias definition in the current statement.
Schema-qualified table names such as =schema.table= are not treated as aliases;
use =C-c C-d= or =C-c C-j= for object lookup and table description.  CAPF
completion handles SQL identifiers, including empty column positions such as
=select | from users=.  =TAB= completes columns after a table or alias dot,
then falls back to indentation elsewhere; other completion stays on normal CAPF
keys such as =M-TAB= inherited from Emacs.
...
| =c=            | Open copy transient menu (choose TSV / CSV / Org table / INSERT / UPDATE; toggle =-a= for all rows/cols, =-r= to refine) |
...
When a query result is not updateable because no row identity was detected,
the same line shows =row identity missing= and =E/D off=; =C-c '= also errors
immediately instead of opening an edit buffer that cannot be committed.
Editable result buffers choose row identity in this order: primary key,
non-null unique key, then backend row locator when available.  Row locator
support is backend-specific: PostgreSQL tables can use =ctid=, SQLite rowid
tables can use =rowid=, and Oracle JDBC tables can use =ROWID=.  Physical row
locators may change after UPDATE, so refresh order is still controlled only by
the SQL query; use an explicit =ORDER BY= when row order matters.
Aggregate or grouped result sets, such as =SELECT count(1) ...=, do not receive
hidden row-identity columns and remain read-only for row mutation commands.
...
The copy transient (=c=) offers two scope switches:

- =-a= (=--all=) — copy every visible row and column in the result set,
  ignoring the current region.
- =-r= (=--refine=) — enter *refine mode* to interactively exclude rows or
  columns before the copy.  Requires an active region.

With an active region, enabling =-r= before choosing a format lets you
interactively exclude rows or columns before the copy is performed.  The
mode-line at the bottom of the result buffer is replaced with a color-coded
hint for the duration of the session.
...
9. s          Sort by current column (SQL ORDER BY), re-queries from page 1
...
- =clutch-copy-context-for-agent= — copy the current SQL, connection context,
  referenced table metadata, and the visible columns from the latest matching
  result sample as Markdown for an external agent; also available as =k= in the
  main and result transients

* External Agent Context

=clutch-copy-context-for-agent= is the lightweight handoff point for ChatGPT, Claude, DeepSeek, or another external assistant.  It copies Markdown to the kill ring
from the current clutch buffer:

- in =clutch-mode=, the active region or current DWIM SQL statement
- in =clutch-result-mode=, the effective result query, including an active
  SQL =WHERE= filter
- connection/backend/schema information
- referenced table metadata from the existing object describe path, including
  comments, columns, primary-key/foreign-key tags, and any warmed related object
  metadata such as indexes
- a bounded sample from the current result buffer, or from the latest result
  buffer produced by the query console when its SQL still matches; hidden row
  identity columns are omitted

The command does not run the SQL being copied.  It only uses clutch metadata
APIs and rows already present in a matching result buffer.  Metadata lookup
errors are written into the copied Markdown instead of being hidden, so the
external agent can see which context is missing.

* Query Console

=clutch-query-console= opens a dedicated buffer =*clutch: NAME*=, enables
=clutch-mode=, and connects automatically.  The picker lists saved
connections; pressing RET with no matching connection starts a temporary connection
flow.  SQLite is part of that flow and reads a database file instead of
host/user credentials.  =clutch-query-sqlite-file= jumps straight to the
SQLite-file shortcut.  Repeated calls with the same saved connection or
temporary connection switch to the existing buffer rather than creating a new
one.  Connection happens before the buffer is shown: if the initial connect or
a reconnect of an existing disconnected console fails, clutch raises the error
without leaving a visible disconnected console buffer behind.

Inside a query console, =C-c C-e= reconnects only that console's current
connection.  For saved consoles, if the saved connection entry was removed from
=clutch-connection-alist=, clutch raises a clear error instead of silently
reconnecting to some other database.  For ad hoc consoles, reconnect uses the
original temporary parameters; SQLite reuses the original file path.  To move to
a different saved or temporary connection, call =clutch-query-console= again;
press RET with no matching connection to create another temporary connection.

In ordinary =clutch-mode= or REPL buffers, =C-c C-e= keeps the generic connect
picker: select a saved connection, or press RET with no matching connection to
enter temporary params.  For SQLite files, use the query-console path instead
of opening the database file itself as an editor buffer.
...
connection is opened.  clutch saves console SQL under a hash of the connection
identity, not the saved connection name.  Renaming a saved connection reuses
the open console buffer and keeps the same saved SQL.  If no identity-keyed file
exists yet, clutch reads the old alias-keyed file as a migration fallback.
...
business schemas such as =sales= and =analytics= within one connection,
...
For JDBC backends, [[https://github.com/LuciusChen/clutch-jdbc-agent][clutch-jdbc-agent]]
now isolates foreground SQL from metadata traffic by keeping separate primary
and metadata JDBC sessions under one logical clutch connection.  This avoids
Oracle-style contention between user queries and background schema/object
introspection.
...
- During long-running queries, press =C-g= to interrupt.
  - On JDBC backends, clutch first sends a protocol-level =cancel= for the
    running statement.
  - On native MySQL, clutch opens a helper connection, issues =KILL QUERY= for
    the original connection id, drains the interrupted response, and keeps the
    same session usable.
...
- =clutch-agent-context-max-result-rows= controls how many current result rows
  are copied by =clutch-copy-context-for-agent=.
- =clutch-agent-context-max-cell-width= controls per-cell truncation in copied
  external-agent context.
