...
- Pinned the published clutch-jdbc-agent 0.2.23 and its verified release checksum. It retains the 0.2.22 SQL diagnostics and value-conversion fixes, reclaims idle per-connection locks, and reuses centralized integer validation for diagnostics.
- Bounded column sizing for long values and reduced repeated list traversal in local aggregation, filtering, and CSV/TSV formatting and copying. INSERT exports resolve the target table once per batch, and Org copying computes widths in one pass. Full table rendering now copies fewer intermediate strings while preserving cell properties and Unicode behavior.
...
- Pinned clutch-jdbc-agent 0.2.19, which keeps the JSON protocol on a dedicated stdout handle and redirects third-party Java `System.out` messages to captured stderr before loading drivers. Snowflake external-browser login status and equivalent JDBC driver console output can no longer corrupt protocol framing.
